...
def test_cast():
    ...
    def run_game(game_factory, stop_event):
        ...
        def fake_get_key_state():
            keys = {value: False for name, value in vars(pl).items() if name.startswith("K_")}
            keys[pygame.K_f] = True
            keys[pygame.K_1] = True
            return keys

        game.get_key_state = fake_get_key_state
        game.main(stop_event=stop_event)

        goblin = [
            obj for obj in game.current_level._observers if obj.__class__.__name__ == "Goblin"
        ][0]
        assert goblin.health < 9000
